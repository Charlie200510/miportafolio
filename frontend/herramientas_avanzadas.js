...
(function() {
  ...
  window.abrirDeepDive = async function(tickerInicial = '') {
    ...
    const ejecutar = async () => {
      ...
      cont.innerHTML = `<p style="text-align:center;color:var(--tinta-4);font-size:13px;padding:20px;">Analizando ${t}… (15-30 seg)</p>`;
      try {
        const res = await fetch(`/api/deep-dive/${encodeURIComponent(t)}`);
        const d = await res.json();
        if (!d.ok) {
          cont.innerHTML = `<p style="color:var(--baja);font-size:13px;">${d.error || 'Error'}</p>`;
          return;
        }
        cont.innerHTML = _renderDeepDive(d);
      } catch (e) {
        cont.innerHTML = `<p style="color:var(--baja);font-size:13px;">Error: ${e.message}</p>`;
      }
    };
  ...
  function _renderDeepDive(d) {
    ...
    const fila = (label, val, fmtFn, compInfo) => {
      ...
      if (compInfo && compInfo.diferencia_pct != null) {
        ...
        const color = pct > 0 ? MP_COLOR.sello : pct < 0 ? MP_COLOR.alza : MP_COLOR.tinta4;
        const signo = pct > 0 ? '+' : '';
    ...
    return `
      <div style="background:var(--sup-panel);border:1px solid rgba(156,93,18,0.25);border-radius:var(--radio-tarjeta);padding:16px;margin-bottom:16px;">
        ...
        ${emp.descripcion ? `<details style="margin-top:10px;"><summary style="cursor:pointer;font-size:11px;color:var(--tinta-4);">Descripción de la empresa</summary><p style="margin:8px 0 0;font-size:12px;color:var(--tinta-3);line-height:1.5;">${emp.descripcion.substring(0, 600)}${emp.descripcion.length > 600 ? '...' : ''}</p></details>` : ''}
      </div>
      ...
      ${peers ? `
        <h4 style="font-size:11px;color:var(--tinta-4);text-transform:uppercase;letter-spacing:0.1em;margin:16px 0 8px;">Peers comparados</h4>
        <div>${peers}</div>
      ` : ''}

      <p style="margin:16px 0 0;font-size:10px;color:var(--tinta-4);text-align:center;font-style:italic;">${d.advertencia || ''}</p>
    `;
  }
  ...
  window.abrirOptimizadorFiscal = async function() {
    const formHTML = `
      <div style="padding:24px;">
        <div style="display:flex;align-items:start;justify-content:space-between;margin-bottom:16px;">
          <div>
            <p style="margin:0;font-size:10px;font-weight:600;letter-spacing:0.18em;text-transform:uppercase;color:var(--sello);">Optimizador fiscal</p>
            <h2 style="margin:4px 0 0;font-size:22px;color:var(--tinta-1);font-weight:700;">¿Qué vender para pagar menos ISR?</h2>
            <p style="margin:6px 0 0;font-size:12px;color:var(--tinta-3);">Algoritmo que decide qué posiciones vender (y en qué orden) para minimizar tu ISR mexicano del 10%.</p>
          </div>
          <button onclick="document.getElementById('mp-herramienta-modal').remove()" style="background:transparent;border:none;color:var(--tinta-4);font-size:24px;cursor:pointer;padding:4px 8px;">×</button>
        </div>
    ...
    document.getElementById('mp-of-calcular').addEventListener('click', async () => {
      ...
      cont.innerHTML = `<p style="text-align:center;color:var(--tinta-4);font-size:13px;padding:14px;">Calculando…</p>`;
      try {
        ...
        const d = await res.json();
        if (!d.ok) {
          cont.innerHTML = `<p style="color:var(--baja);font-size:13px;padding:14px;">${d.error || 'Error'}</p>`;
          return;
        }
  ...
  function _renderOptimizador(d) {
    const plan = (d.plan || []).map((p, i) => {
      const color = p.categoria === 'pérdida' ? MP_COLOR.alza :
                    p.categoria === 'ganancia' ? MP_COLOR.baja : MP_COLOR.tinta4;
      const signo = p.ganancia_realizada >= 0 ? '+' : '';
      return `
        <tr>
          <td style="padding:8px 12px;color:var(--tinta-3);font-size:12px;text-align:center;">${i+1}</td>
          <td style="padding:8px 12px;color:var(--tinta-1);font-weight:600;font-family:monospace;font-size:13px;">${p.ticker}</td>
          <td style="padding:8px 12px;text-align:right;font-family:monospace;color:var(--tinta-2);font-size:12px;">${p.shares_vender.toFixed(2)}</td>
          <td style="padding:8px 12px;text-align:right;font-family:monospace;color:var(--tinta-2);font-size:12px;">$${p.monto_mxn.toLocaleString()}</td>
    ...
    return `
        ...
        <table style="width:100%;border-collapse:collapse;background:var(--sup);border:1px solid var(--sup-panel);border-radius:var(--radio);">
          <thead>
            <tr style="background:var(--sup-panel);">
              ...
              <th style="padding:8px 12px;font-size:10px;color:var(--tinta-4);text-transform:uppercase;text-align:left;">Ticker</th>
              <th style="padding:8px 12px;font-size:10px;color:var(--tinta-4);text-transform:uppercase;text-align:right;">Shares</th>
              <th style="padding:8px 12px;font-size:10px;color:var(--tinta-4);text-transform:uppercase;text-align:right;">$ MXN</th>
              <th style="padding:8px 12px;font-size:10px;color:var(--tinta-4);text-transform:uppercase;text-align:right;">Ganancia</th>
              <th style="padding:8px 12px;font-size:10px;color:var(--tinta-4);text-transform:uppercase;text-align:center;">Cat.</th>
            </tr>
          </thead>
          <tbody>${plan || '<tr><td colspan="6" style="padding:20px;text-align:center;color:var(--tinta-4);">Sin plan generado</td></tr>'}</tbody>
        </table>
      </div>
      ...
      <div style="margin-top:14px;padding:12px;background:rgba(26,26,24,.12);border:1px solid var(--sup-panel);border-radius:var(--radio-tarjeta);font-size:11px;color:var(--tinta-3);line-height:1.6;">
        ...
        ${d.perdida_arrastrable > 0 ? `<p style="margin:0;color:var(--alza);"><strong>Pérdida arrastrable a años futuros:</strong> $${d.perdida_arrastrable.toLocaleString()}</p>` : ''}
      </div>
      <p style="margin:12px 0 0;font-size:10px;color:var(--tinta-4);text-align:center;font-style:italic;">${d.advertencia}</p>
    `;
  }
  ...
  window.abrirScreener = function() {
    const formHTML = `
      <div style="padding:24px;">
        <div style="display:flex;align-items:start;justify-content:space-between;margin-bottom:16px;">
          <div>
            <p style="margin:0;font-size:10px;font-weight:600;letter-spacing:0.18em;text-transform:uppercase;color:var(--sello);">Screener</p>
            <h2 style="margin:4px 0 0;font-size:22px;color:var(--tinta-1);font-weight:700;">Filtra el universo de tickers</h2>
            <p style="margin:6px 0 0;font-size:12px;color:var(--tinta-3);">Encuentra acciones que cumplan tus criterios: P/E, yield, beta, market cap, sector...</p>
          </div>
          <button onclick="document.getElementById('mp-herramienta-modal').remove()" style="background:transparent;border:none;color:var(--tinta-4);font-size:24px;cursor:pointer;padding:4px 8px;">×</button>
        </div>
    ...
    document.getElementById('mp-sc-buscar').addEventListener('click', async () => {
      ...
      cont.innerHTML = `<p style="text-align:center;color:var(--tinta-4);font-size:13px;padding:14px;">Filtrando…</p>`;
      ...
      try {
        ...
        const d = await res.json();
        if (!d.ok) { cont.innerHTML = `<p style="color:var(--baja);">${d.error || 'Error'}</p>`; return; }
        cont.innerHTML = _renderScreener(d);
      } catch (e) { cont.innerHTML = `<p style="color:var(--baja);">Error: ${e.message}</p>`; }
    });
  };
  ...
  window.abrirAlertasAvanzadas = function() {
    ...
    const html = `
      <div style="padding:24px;">
        <div style="display:flex;align-items:start;justify-content:space-between;margin-bottom:16px;">
          <div>
            <p style="margin:0;font-size:10px;font-weight:600;letter-spacing:0.18em;text-transform:uppercase;color:var(--alza);">Alertas avanzadas</p>
            <h2 style="margin:4px 0 0;font-size:22px;color:var(--tinta-1);font-weight:700;">Reglas multi-condición</h2>
            <p style="margin:6px 0 0;font-size:12px;color:var(--tinta-3);">Crea reglas tipo "avísame si NVDA cae &lt;100 Y el VIX &gt; 25".</p>
          </div>
          <button onclick="document.getElementById('mp-herramienta-modal').remove()" style="background:transparent;border:none;color:var(--tinta-4);font-size:24px;cursor:pointer;padding:4px 8px;">×</button>
        </div>

        <h4 style="font-size:11px;color:var(--tinta-4);text-transform:uppercase;letter-spacing:0.1em;margin:0 0 8px;">Tus reglas (${reglas.length})</h4>
  ...
  window.abrirBackupsNube = async function() {
    ...
    const html = `
      <div style="padding:24px;">
        <div style="display:flex;align-items:start;justify-content:space-between;margin-bottom:16px;">
          <div>
            <p style="margin:0;font-size:10px;font-weight:600;letter-spacing:0.18em;text-transform:uppercase;color:var(--sello);">Backups</p>
            <h2 style="margin:4px 0 0;font-size:22px;color:var(--tinta-1);font-weight:700;">Backups en la nube</h2>
            <p style="margin:6px 0 0;font-size:12px;color:var(--tinta-3);">Guarda snapshots de tu portafolio para restaurar si pierdes el navegador o quieres revertir cambios.</p>
            <p style="margin:6px 0 0;font-size:11px;color:var(--sello);font-family:monospace;"><span class="mp-marca" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="square" stroke-linejoin="miter"><rect x="2.5" y="5" width="19" height="14"/><path d="M2.5 6l9.5 7 9.5-7"/></svg></span> ${email}</p>
          </div>
          <button onclick="document.getElementById('mp-herramienta-modal').remove()" style="background:transparent;border:none;color:var(--tinta-4);font-size:24px;cursor:pointer;padding:4px 8px;">×</button>
        </div>
        <div style="display:flex;gap:8px;margin-bottom:16px;">
    ...
    async function recargar() {
      ...
      cont.innerHTML = `<p style="color:var(--tinta-4);font-size:12px;text-align:center;padding:14px;">Cargando…</p>`;
      try {
        const res = await fetch(`/api/backups?email=${encodeURIComponent(email)}`);
        const d = await res.json();
        if (!d.ok) { cont.innerHTML = `<p style="color:var(--baja);">${d.error}</p>`; return; }
        if (!d.backups || !d.backups.length) {
          cont.innerHTML = `<p style="text-align:center;color:var(--tinta-4);font-size:13px;padding:24px;">Aún no tienes backups. Crea uno arriba.</p>`;
          return;
        }
        ...
        cont.innerHTML = `<p style="color:var(--baja);">Error: ${e.message}</p>`;
      }
    }
  ...
  window.abrirConfigPush = function() {
    ...
    const html = `
      <div style="padding:24px;">
        <div style="display:flex;align-items:start;justify-content:space-between;margin-bottom:16px;">
          <div>
            <p style="margin:0;font-size:10px;font-weight:600;letter-spacing:0.18em;text-transform:uppercase;color:var(--alza);">Notificaciones</p>
            <h2 style="margin:4px 0 0;font-size:22px;color:var(--tinta-1);font-weight:700;">Push notifications</h2>
            <p style="margin:6px 0 0;font-size:12px;color:var(--tinta-3);">Recibe alertas instantáneas en tu teléfono/desktop (sin email) cuando se disparen tus reglas.</p>
          </div>
          <button onclick="document.getElementById('mp-herramienta-modal').remove()" style="background:transparent;border:none;color:var(--tinta-4);font-size:24px;cursor:pointer;padding:4px 8px;">×</button>
        </div>
        ...
        <p style="margin:0 0 14px;font-size:12px;color:var(--tinta-3);line-height:1.55;">
          <strong style="color:var(--tinta-1);">Requisitos:</strong><br>
  ...
  function _renderSml(d) {
    ...
    const alphaColor = d.alpha >= 0 ? MP_COLOR.alza : MP_COLOR.baja;
    ...
    return `
      ...
      <div style="background:var(--sup);border:1px solid var(--regla);border-radius:var(--radio-tarjeta);padding:16px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px;font-size:12px;">
          <div style="padding:6px 0;border-bottom:1px solid var(--sup-panel);">
            <span style="color:var(--tinta-4);">Benchmark:</span>
            <span style="color:var(--tinta-1);font-family:monospace;float:right;">${d.benchmark}</span>
          </div>
          <div style="padding:6px 0;border-bottom:1px solid var(--sup-panel);">
            <span style="color:var(--tinta-4);">Moneda:</span>
          ...
          <div style="padding:6px 0;border-bottom:1px solid var(--sup-panel);">
            <span style="color:var(--tinta-4);">Premio del mercado:</span>
            <span style="color:var(--tinta-1);font-family:monospace;float:right;">${fmtPct(d.premio_mercado)}</span>
          </div>
          <div style="padding:6px 0;border-bottom:1px solid var(--sup-panel);">
            <span style="color:var(--tinta-4);">Retorno mercado (real):</span>
            <span style="color:var(--tinta-1);font-family:monospace;float:right;">${fmtPctSigno(d.retorno_mercado_anual)}</span>
          </div>
          <div style="padding:6px 0;border-bottom:1px solid var(--sup-panel);">
            <span style="color:var(--tinta-4);">Periodo:</span>
      ...
      <p style="margin:0;font-size:10px;color:var(--tinta-4);text-align:center;font-style:italic;">${d.advertencia}</p>
    `;
  }
  ...
  window.renderSmlEn = async function(ticker, contId) {
    ...
    cont.innerHTML = `<p style="text-align:center;color:var(--tinta-4);font-size:13px;padding:18px;">Calculando valoración SML…</p>`;
    try {
  ...
  window.renderDeepDiveEn = async function(ticker, contId) {
    const cont = document.getElementById(contId);
    if (!cont || !ticker) return;
    cont.innerHTML = `<p style="text-align:center;color:var(--tinta-4);font-size:13px;padding:18px;">Analizando a fondo… (puede tardar unos segundos)</p>`;
    ...
    try {
      ...
      const msg = e.name === 'AbortError'
        ...
        : 'Error: ' + e.message;
      cont.innerHTML = `<p style="color:var(--tinta-4);font-size:12px;padding:10px;">${msg} <button onclick="window.renderDeepDiveEn('${ticker}','${contId}')" style="color:var(--sello);text-decoration:underline;cursor:pointer;">Reintentar</button></p>`;
    }
  };
