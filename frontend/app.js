...
// El módulo 'Tu mes' (resumen mensual estilo Spotify Wrapped) se retiró:
// un pop-up automático el día 1 de cada mes con diapositivas y descarga de
// imagen. Con él se fue html2canvas, que era su único consumidor y venía de
// un CDN externo.
  ...
  function render(d) {
    ...
    if (_tipo === 'etf' || _tipo === 'crypto') {
      ...
      cont.innerHTML = headerHTML + fundHTML + chartHTML + peerHTML
                     ...
                     + smlHTML + mmHTML + estHostHTML + ddHostHTML;
    }
    ...
    if (_tipo === 'accion') {
      if (window.renderSmlEn) window.renderSmlEn(d.ticker, 'an-sml-host');
        if (window.renderMMEn) window.renderMMEn(d.ticker, 'an-mm-host');
